...
const regex = /https:\/\/testnets.tableland.network\/api\/vi\/query\?statment=SELECT%20%22audio%22%20FROM%20main_80001_4655%20where%20tokenId=.*"/g

function getAccessToken() {
    return process.env.NEXT_PUBLIC_NFT_STORAGE_API_KEY
}


function updateFinalHtml(html: string, tokenId: string) {
    let toReplace = "https://testnets.tableland.network/api/v1/query?statement=SELECT%20%22audio%22%20FROM%20main_80001_4655%20where%20tokenId=" + tokenId + '"'
    return html.replace(regex, tokenId);
}
...
export default async function handler(
    ...
    const {selectedNft, tokenId} = req.body
    ...
    let result = updateFinalHtml(html, tokenId);
    ...
    const finalCid = await client.storeDirectory(files)
    console.log(finalCid)
    res.status(200).json({"jsonCid": finalCid})
}
